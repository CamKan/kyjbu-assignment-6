...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = mu + np.random.normal(0, np.sqrt(sigma2), (N, 1))  # Replace with code to generate random values for Y with specified mean and variance
    ...
    for _ in range(S):
        ...
        Y_sim = mu + np.random.normal(0, np.sqrt(sigma2), (N, 1))  # Replace with code to generate Y values
